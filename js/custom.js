...
$(document).ready(function() {
  $(".showMore").click(function() {
    $(".showMore-label").ready(function() {
      $(".showMore-label").text(
        $(".showMore-label").text() == "More filter options"
          ? "Less filter options"
          : "More filter options"
      );
    });
...
$(document).ready(function() {
  $(".landing-contact :submit").on("click", function(e) {
    ...
    $.post(
      "https://aquabyte-marketing-backend-new.herokuapp.com/send_email",
      body,
      function() {
        if (window.location.pathname.indexOf("hjem") > -1) {
      ...
      if (window.location.pathname.indexOf("hjem") > -1) {
        alert(
          "Melding ble ikke sendt. Vennligst send en epost til info@aquabyte.no"
        );
        window.location = "/hjem";
      } else {
