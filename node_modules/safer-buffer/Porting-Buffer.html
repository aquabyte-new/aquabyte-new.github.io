...
a(href="/")
	img(src="/img/flag-usa.svg")
--></ul></div></div></header><h1>Porting to the Buffer.from/Buffer.alloc API</h1><p><a id="overview"></a></p>
<h2>Overview</h2><ul>
...
<h3>Finding problematic bits of code using grep</h3><p>Just run <code>grep -nrE &#39;[^a-zA-Z](Slow)?Buffer\s*\(&#39; --exclude-dir node_modules</code>.</p>
<p>It will find all the potentially unsafe places in your own code (with some considerably unlikely
exceptions).</p>
<h3>Finding problematic bits of code using Node.js 8</h3><p>If you’re using Node.js ≥ 8.0.0 (which is recommended), Node.js exposes multiple options that help with finding the relevant pieces of code:</p>
<ul>
...
(node:7147) [DEP0005] DeprecationWarning: The Buffer() and new Buffer() constructors are not recommended for use due to security and usability concerns. Please use the new Buffer.alloc(), Buffer.allocUnsafe(), or Buffer.from() construction methods instead.
    ...
    [... more stack trace lines ...]</code></pre>
<h3>Finding problematic bits of code using linters</h3><p>Eslint rules <a href="https://eslint.org/docs/rules/no-buffer-constructor">no-buffer-constructor</a>
or
...
<h2>Variant 1: Drop support for Node.js ≤ 4.4.x and 5.0.0 — 5.9.x.</h2><p>This is the recommended solution nowadays that would imply only minimal overhead.</p>
...
<h2>Variant 2: Use a polyfill</h2><p>Utilize <a href="https://www.npmjs.com/package/safer-buffer">safer-buffer</a> as a polyfill to support older
Node.js versions.</p>
...
<h2>Variant 3 — manual detection, with safeguards</h2><p>This is useful if you create Buffer instances in only a few places (e.g. one), or you have your own
wrapper around them.</p>
<h3>Buffer(0)</h3><p>This special case for creating empty buffers can be safely replaced with <code>Buffer.concat([])</code>, which
returns the same result all the way down to Node.js 0.8.x.</p>
<h3>Buffer(notNumber)</h3><p>Before:</p>
<pre><code class="language-js">var buf = new Buffer(notNumber, encoding);</code></pre>
<p>After:</p>
...
compiles to.</p>
<h3>Buffer(number)</h3><p>For Node.js 0.10.x (and below) support:</p>
<pre><code class="language-js">var buf;
if (Buffer.alloc) {
...
<pre><code class="language-js">const buf = Buffer.alloc ? Buffer.alloc(number) : new Buffer(number).fill(0);</code></pre>
<h2>Regarding Buffer.allocUnsafe</h2><p>Be extra cautious when using <code>Buffer.allocUnsafe</code>:</p>
<ul>
...
<p><a id="faq"></a></p>
<h2>FAQ</h2><p><a id="design-flaws"></a></p>
<h3>What is wrong with the <code>Buffer</code> constructor?</h3><p>The <code>Buffer</code> constructor could be used to create a buffer in many different ways:</p>
<ul>
...
}</code></pre><p>Note that this code does <em>not</em> validate the type of <code>req.body.string</code>:</p>
<ul>
